...
public class ClassRoomIdGenerator implements IdentifierGenerator {
	...
	public Serializable generate(SharedSessionContractImplementor session, Object object) throws HibernateException {
		...
		try {
			...
			String queryString = "from ClassRoom order by classroomId desc";
			Query query = newSession.createQuery(queryString);
			ClassRoom classRoom = (ClassRoom) query.setMaxResults(1).uniqueResult();

			if (classRoom != null) {
				int count = Integer.parseInt(classRoom.getClassroomId().replace(prefix, ""));
				int present = count + 1;
